    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i in data) {
                ...
                icon = icon.replace(/http:\/\//g, "https://");
                uSinaEmotionsHt.put(data[i].phrase, icon);
            }
        }
    });
});
    ...
    $.fn.SinaEmotion = function (target) {
        ...
        function showEmotions() {
            ...
            for (var i = 0; i < emotions[category].length; ++i) {
                var icon = emotions[category][i].icon;
                icon = icon.replace(/http:\/\//g, "https://");
                $('#emotions .container').append($('<a href="javascript:void(0);" title="' + emotions[category][i].name + '"><img src="' + icon + '" alt="' + emotions[category][i].name + '" width="22" height="22" /></a>'));
            }
